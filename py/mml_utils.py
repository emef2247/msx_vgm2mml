...
def emit_volume(new_v, old_v):
    """Return the shortest MML token to change volume from *old_v* to *new_v*.

    Convention (matches scc.mml.tcl ``get_volume_diff_in_mml``):
      ``)``: decrease volume by 1 per character
      ``(``: increase volume by 1 per character

    If the relative token is strictly shorter than the absolute ``v{new_v}``
    token, the relative form is returned.  When lengths are equal the absolute
    form is preferred.  Returns ``''`` when no change is needed.
    """
    if new_v == old_v:
        return ''
    diff = new_v - old_v
    rel = ')' * (-diff) if diff < 0 else '(' * diff
    abs_tok = f'v{new_v}'
    return rel if len(rel) < len(abs_tok) else abs_tok


def emit_octave(new_o, old_o):
    """Return the shortest MML token to change octave from *old_o* to *new_o*.

      ``>``: increase octave by 1 per character
      ``<``: decrease octave by 1 per character

    Relative form is returned only when strictly shorter than ``o{new_o}``.
...
def rle_compress_tokens(tokens):
    """Apply run-length encoding to a list of MML token strings.

    Consecutive runs of the same token ``T`` repeated ``N`` times are replaced
    by ``N[T]`` when that string is strictly shorter than the original ``N``
    space-separated copies.  Runs that cannot be compressed are kept as-is.

    Example::

        >>> rle_compress_tokens([')e%1', ')e%1', ')e%1'])
        ['3[)e%1]']
        >>> rle_compress_tokens(['e%4', 'e%4'])
        ['2[e%4]']
        >>> rle_compress_tokens(['a'])  # single token unchanged
        ['a']
    """
    if not tokens:
        return list(tokens)
    ...
    while i < len(tokens):
        ...
        count = j - i
        if count > 1:
            # Cost of N space-separated copies: N*len(tok) + (N-1) spaces
            orig_len = count * len(tok) + (count - 1)
            compressed = f'{count}[{tok}]'
